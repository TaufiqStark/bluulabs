...
export const metadata = {
  title: "Writings | Admin - taufiq.",
};

export default async function WritingsAdminPage() {
  const data = await db.select().from(writings).orderBy(desc(writings.createdAt));

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight admin-text">Writings</h1>
          <p className="mt-1 text-sm admin-muted">Manage blog posts and articles.</p>
        </div>
        <Link
          ...
          <table className="min-w-full divide-y divide-gray-300">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Excerpt</th>
              ...
              {data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 space-x-4">
                    <Link href={`/admin/writing/${item.id}/edit`} className="text-blue-600 hover:text-blue-900">
                      Edit
                    </Link>
